...
PROJECT_DIR="$APPLICATIONS_DIR/hyprlock"

# Ensure necessary directories exist
mkdir -p "$APPLICATIONS_DIR"

git clone https://github.com/hyprwm/hyprlock.git "$PROJECT_DIR"
...
# Build the project
cmake --build ./build --config Release --target hyprlock -j`nproc 2>/dev/null || getconf NPROCESSORS_CONF` || { echo "Error: Build failed."; exit 1; }
